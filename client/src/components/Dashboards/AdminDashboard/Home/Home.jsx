...
function Home() {
  ...
  const getComplaints = async () => {
    ...
    const response = await fetch(`http://localhost:5000/api/complaint/hostel`, {
      method: "POST",
      headers: {
  ...
  const getSuggestions = async () => {
    ...
    const response = await fetch(
      "http://localhost:5000/api/suggestion/hostel",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const getRequests = async () => {
    const hostel = JSON.parse(localStorage.getItem("hostel"));
    const res = await fetch("http://localhost:5000/api/messoff/list", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${localStorage.getItem("token")}`
