...
function Suggestions() {
  const getSuggestions = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/suggestion/hostel", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        toast.error(data.errors?.[0]?.msg || "Failed to fetch suggestions", {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const updateSuggestion = async (id) => {
    try {
      setLoader(true);
      const response = await fetch("http://localhost:5000/api/suggestion/update", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
      ...
      if (data.success) {
        ...
        toast.error(data.errors?.[0]?.msg || "Failed to update suggestion", {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
