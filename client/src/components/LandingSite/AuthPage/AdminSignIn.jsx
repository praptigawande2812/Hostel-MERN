...
export default function AdminSignIn() {
  ...
  const getHostel = async () => {
    ...
    try {
      console.log('Getting hostel for admin:', admin); // Debug log
      const res = await fetch("http://localhost:5000/api/admin/get-hostel", {
        method: "POST",
        headers: {
      ...
      console.log('Hostel response:', data); // Debug log

      if (data.success) {
        localStorage.setItem("hostel", JSON.stringify(data.hostel));
      } else {
        console.error('Error getting hostel:', data.errors); // Debug log
        toast.error(
          data.errors[0].msg, {
          position: "top-right",
          autoClose: 3000,
      ...
      console.error('Error in getHostel:', err); // Debug log
      toast.error(
        'Error getting hostel data', {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  let login = async (event) => {
    ...
    if (result.success) {
      ...
      let admin = await fetch("http://localhost:5000/api/admin/get-admin", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
