...
function Mess() {
  ...
  const requestMessOff = async (event) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/Messoff/request", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.errors?.[0]?.msg || "Failed to request leave");
      }
  ...
  useEffect(() => {
    const fetchMessOffData = async () => {
      try {
        ...
        const response = await fetch("http://localhost:5000/api/Messoff/count", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${localStorage.getItem("token")}`
        ...
        if (!response.ok) {
          throw new Error(result.errors?.[0]?.msg || "Failed to fetch leave data");
        }
