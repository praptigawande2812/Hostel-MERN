...
function AllStudents() {
  const getCSV = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/student/csv", {
        method: "POST",
        headers: {
      ...
      if (!data.success) {
        throw new Error(data.errors?.[0]?.msg || "Failed to generate CSV");
      }
      ...
      toast.error(error.message || "Failed to download student list", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
