...
function MessOff() {
  const getRequests = async () => {
    setProgress(30);
    try {
      const res = await fetch("http://localhost:5000/api/messoff/all", {
        method: "GET",
        headers: {
      ...
      if (data.success) {
        ...
        toast.error(data.errors?.[0]?.msg || "Failed to fetch mess off requests");
      }
    } catch (error) {
      toast.error("Failed to fetch mess off requests");
    }
    setProgress(100);
  };

  const updateRequest = async (id, status) => {
    const res = await fetch("http://localhost:5000/api/messoff/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${localStorage.getItem("token")}`
