...
function RegisterStudent() {
  const registerStudent = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/student/register-student", {
        method: "POST",
        headers: {
        ...
        <form method="post" onSubmit={registerStudent} className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-5 w-full justify-center">
            ...
            <div className="flex flex-col">
              <label htmlFor="hostel" className="block mb-2 text-sm font-medium text-white">
                Hostel
              </label>
              <select
                name="hostel"
              ...
              >
                {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
                  <option key={num} value={`Hostel ${num}`}>
                    Hostel {num}
                  </option>
                ))}
              </select>
