...
function Attendance() {
  ...
  const getALL = async () => {
    setProgress(30);
    const marked = await fetch("http://localhost:5000/api/attendance/getHostelAttendance", {
      method: "POST",
      headers: {
    ...
    if (markedData.success) {
      const markedStudents = markedData.attendance.map((attendance) => ({
        id: attendance.student._id,
        cms: attendance.student.cms_id,
        name: attendance.student.name,
        room: attendance.student.room_no,
      ...
      const unmarkedStudents = students
        .filter(student => !markedStudents.find(marked => marked.id === student._id))
        .map(student => ({
          id: student._id,
          cms: student.cms_id,
          name: student.name,
  ...
  const markAttendance = async (id, isPresent) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.errors?.[0]?.msg || "Failed to mark attendance");
      }
      ...
      if (data.success) {
        ...
        // Update the UI
        const updatedUnmarkedStudents = unmarkedStudents.map(student => {
          if (student.id === id) {
        ...
        setunmarkedStudents(updatedUnmarkedStudents.filter(student => student.attendance === undefined));
        setMarkedStudents(prev => [
          ...prev,
          ...updatedUnmarkedStudents.filter(student => student.attendance !== undefined)
        ]);
